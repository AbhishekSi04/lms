...
export const verifyUserPayment = createAsyncThunk("/payments/verify", async (data) => {
    ...
    try {
        console.log("Redux: Sending payment verification data:", data);
        const response = await axiosInstance.post("/payments/verify", data);
        console.log("Redux: Payment verification response:", response?.data);
        toast.success("Payment verified", { id: loadingId });
        return response?.data;
    } catch (error) {
        console.error("Redux: Payment verification error:", error?.response?.data);
...
const razoraySlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(purchaseCourseBundle.fulfilled, (state, action) => {
            console.log("Redux: Setting subscription_id to:", action?.payload?.order_id);
            state.subscription_id = action?.payload?.order_id
        })
